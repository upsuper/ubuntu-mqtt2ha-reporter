...
pub fn get() -> Result<Uuid, Error> {
    let machine_id = fs::read_to_string(MACHINE_ID_FILE).context("Failed to read machine-id file")?;
    let machine_id = Uuid::try_parse(machine_id.trim_end()).context("Failed to parse machine-id as UUID")?;
